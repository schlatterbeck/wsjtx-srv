...
class QDateTime(Protocol_Element):
    ...
    def __str__(self):
        s = (
            "QDatTime(date=%(date)s time=%(time)s "
...
class QColor(Protocol_Element):
    ...
    def __init__(self, red=0, green=0, blue=0, alpha=cmax, spec=spec_rgb):
        self.spec = spec
        self.red = red
        self.green = green
        self.blue = blue
    ...
    def serialize(self):
        return pack(self.fmt, self.spec, self.alpha, self.red, self.green, self.blue, 0)
    ...
    def __str__(self):
        ...
        s = "QColor(alpha=%(alpha)s, red=%(red)s, " + "green=%(green)s, blue=%(blue)s)"
        return s % self.__dict__
...
class WSJTX_Heartbeat(WSJTX_Telegram):
    ...
    defaults = dict(max_schema=3, version="", revision="", **WSJTX_Telegram.defaults)
...
class WSJTX_Free_Text(WSJTX_Telegram):
    type = 9
    format = WSJTX_Telegram.format + [("text", qutf8), ("send", qbool)]
...
class UDP_Connector:
    ...
    re_call = re.compile(r"(([A-Z])|([A-Z][A-Z0-9])|([0-9][A-Z]))[0-9][A-Z]{1,3}")
...
class QSO_Database_Worked_Before(Worked_Before):
    ...
    def __init__(
        self, url, username, locator, adif=None, args=None, password=None, **kw
    ):
        d = dict(url=url, username=username, password=password, dry_run=True)
        if "verbose" in kw:
    ...
    def add_dxccs(self):
        ...
        if len(hc) != 1:
            raise ValueError(
                "Ham Call with Loc=%s: Got %d entries" % (self.locator, len(hc))
            )
...
def default_cmd(defaults=None):
    ...
    cmd.add_argument("-p", "--password", help="Password, better use .netrc")
    cmd.add_argument(
        "-U",
        "--dburl",
...
def wbf(argv=None):
    cmd = default_cmd()
    cmd.add_argument("callsign", help="Callsign(s) to look up", nargs="+")
    cmd.add_argument(
        "-b", "--band", help="Band to do lookup for, default=%(default)s", default="40m"
    )
    cmd.add_argument(
        "-D",
        "--use-dxcc",
